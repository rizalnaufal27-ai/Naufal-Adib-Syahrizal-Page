            ...
            >
                ...
                <div className="sticky top-0 z-10 p-5 flex items-center justify-between" style={{ background: "rgba(10,10,10,0.98)", borderBottom: "1px solid var(--color-border)" }}>
                    <h2 className="text-lg font-bold" style={{ color: "var(--color-text)" }}>
                        {step === "pricing" && t("steps.pricing")}
                        {step === "details" && t("steps.details")}
                        {step === "review" && t("steps.review")}
                        {step === "success" && t("steps.success")}
                    </h2>
                ...
                <div className="p-5">
                    ...
                    {step === "details" && (
                        <div className="space-y-4">
                            <div>
                                <label className="text-xs font-semibold block mb-1.5" style={{ color: "var(--color-text-muted)" }}>{t("fields.name")}</label>
                            ...
                            <div>
                                <label className="text-xs font-semibold block mb-1.5" style={{ color: "var(--color-text-muted)" }}>{t("fields.email")}</label>
                                <input type="email" value={form.email} onChange={(e) => handleChange("email", e.target.value)} required
                                    className="w-full px-4 py-3 rounded-xl text-sm outline-none focus:ring-2 focus:ring-indigo-500/30"
                                    style={{ background: "rgba(255,255,255,0.03)", border: "1px solid var(--color-border)", color: "var(--color-text)" }} />
                            </div>
                            <div>
                                <label className="text-xs font-semibold block mb-1.5" style={{ color: "var(--color-text-muted)" }}>{t("fields.description")}</label>
                            ...
                            <div className="flex gap-3">
                                <button onClick={() => setStep("pricing")} className="px-6 py-3 rounded-xl text-sm font-medium" style={{ color: "var(--color-text-muted)", border: "1px solid var(--color-border)" }}>← {t("actions.back")}</button>
                    ...
                    {step === "review" && (
                        <div className="space-y-4">
                            <div className="rounded-xl p-4 space-y-2" style={{ background: "rgba(255,255,255,0.02)", border: "1px solid var(--color-border)" }}>
                                <Row label={t("review.service")} value={serviceLabel} />
                                <Row label={t("review.estimatedPrice")} value={pricing.formatPrice(totalUSD)} bold />
                                <Row label={t("fields.name")} value={form.name} />
                                <Row label={t("fields.email")} value={form.email} />
                                {form.description && <Row label={t("fields.description")} value={form.description} />}
                            </div>

                            <div className="rounded-xl p-4" style={{ background: "rgba(99,102,241,0.06)", border: "1px solid rgba(99,102,241,0.2)" }}>
                                <div className="text-xs space-y-1" style={{ color: "var(--color-text-muted)" }}>
                                    <p>🔹 {t("review.downPayment")}: <strong style={{ color: "#6366F1" }}>{pricing.formatPrice(Math.ceil(totalUSD * 0.2))}</strong></p>
                                    <p>🔹 {t("review.finalPayment")}: <strong style={{ color: "var(--color-text)" }}>{pricing.formatPrice(Math.ceil(totalUSD * 0.8))}</strong></p>
                                </div>
                            </div>
                            ...
                            <div className="flex gap-3">
                                <button onClick={() => setStep("details")} className="px-6 py-3 rounded-xl text-sm font-medium" style={{ color: "var(--color-text-muted)", border: "1px solid var(--color-border)" }}>← {t("actions.back")}</button>
                                <button onClick={handleSubmit} disabled={loading} className="flex-1 py-3 rounded-xl text-sm font-semibold text-white disabled:opacity-50 transition-all"
                                    style={{ background: "linear-gradient(135deg, #6366F1, #4F46E5)" }}>
                                    {loading ? t("actions.placingOrder") : t("actions.placeOrder")}
                                </button>
                            </div>
                        </div>
                    )}
                    ...
                    {step === "success" && orderResult && (
                        <div className="text-center space-y-5 py-6">
                            ...
                            <div>
                                <p className="text-lg font-bold" style={{ color: "var(--color-text)" }}>{t("success.title", { orderNumber: orderResult.order_number })}</p>
                                <p className="text-sm mt-1" style={{ color: "var(--color-text-muted)" }}>{t("success.subtitle")}</p>
                            </div>
                            <div className="rounded-xl p-4 text-left" style={{ background: "rgba(255,255,255,0.02)", border: "1px solid var(--color-border)" }}>
                                <p className="text-xs mb-2" style={{ color: "var(--color-text-muted)" }}>{t("success.privateLink")}</p>
